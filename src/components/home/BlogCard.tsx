...
export default function BlogCard({ title, image, slug, category, eventDate, subtitle }) {
  return (
<article className="flex flex-col bg-white dark:bg-gray-900 shadow-lg rounded-md transition duration-500 ease-in-out transform hover:-translate-y-2">
  <Link href={`/events/${slug}`} aria-label={title}>
    ...
    <div className="flex flex-col h-60 p-6 justify-between">
      ...
      <div>
        <span className="inline-block bg-green bg-opacity-10 rounded-full px-3 py-1 text-sm font-semibold text-green-700 mr-2 mb-2">{eventDate}</span>
      </div>
    </div>
  </Link>
</article>
